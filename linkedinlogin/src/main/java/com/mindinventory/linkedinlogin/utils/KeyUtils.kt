...
object KeyUtils {
    ...
    const val SCOPE_VALUE_KEY = "BOTH_EMAIL_USERDETAILS_SCOPE_VALUE"
    const val BEARER = "Bearer"
    const val BOTH_EMAIL_USERDETAILS_SCOPE_VALUE = "r_liteprofile%20r_emailaddress%20w_member_social"
    const val REDIRECT_URI_KEY = "redirect_uri"
    const val CLIENT_ID_KEY = "client_id"
    const val RESPONSE_TYPE = "response_type"
    const val COMMON_URL = "https://www.linkedin.com/oauth/v2/authorization?"
    const val AUTHORIZATION_CODE = "authorization_code"
    const val AUTHORIZATION_VALUE = "application/x-www-form-urlencoded"
    const val KEY_LINKEDIN_CONTENT = "key_linkedin_detail_data"
    const val REQUEST_CODE = 101
    const val ONLY_EMAIL_SCOPE="r_emailaddress"
    const val ONLY_PROFILE_SCOPE="r_liteprofile"
